...
function DesktopNavigation(props: Props) {
      ...
      <li>
        <Link href="/" className="flex flex-row justify-center">
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <div className={`${farmhouse.className} fs-200 whitespace-nowrap`}>
              Constellation d&apos;artistes
            </div>
          </div>
          <Image src="/circle-icon.png" width={150} height={150} alt="Céphée" />
        </Link>
      </li>
    );
  };
  ...
  return (
    <ul className="DesktopNavigation fs-300 h-full grid grid-cols-3 items-center capitalize ">
      <div className="flex">
        {props.links
          .slice(0, props.links.length / 2 + 1)
          .map((link: { href: string; label: string }) => NavLink(link))}
      </div>
      {mainLink()}
      <div className="flex">
        {props.links
          .slice(props.links.length / 2 + 1, props.links.length)
          .map((link: { href: string; label: string }) => NavLink(link))}
      </div>
    </ul>
  );
